...
# TODO: 
# PROBLEM 1: Make Pacman's mouth work
#
# Somewhere in this file, make the necessary adjustments
# to make Pacman's mouth open and close. Your work during the lab should
# give you an understanding of how the mouth works, but read the code
# carefully to track down the necessary change. There are TWO small 
...
class Pacman(GameCharacter):
    ...
    def __init__(self, maze, game_controller):
        ...
        # modification 1: default value from 0 to 45
        self.mouth_angle = 45 
        self.open_close = 0
        self.rot_begin = 0
        self.rot_end = 360
    ...
    def update(self):
        """Carry out necessary updates for each frame before drawing to screen"""
        self.on_left = abs(self.x - self.maze.LEFT_VERT) < self.WALL_TOLERANCE
        self.on_right =  abs(self.x - self.maze.RIGHT_VERT) < self.WALL_TOLERANCE
        #self.on_vert = self.on_left or self.on_right
        
        self.on_top = abs(self.y - self.maze.TOP_HORIZ) < self.WALL_TOLERANCE
        self.on_bottom = abs(self.y - self.maze.BOTTOM_HORIZ) < self.WALL_TOLERANCE
        #self.on_horiz = self.on_top or self.on_bottom
        
        # modification 2: from < to >=
