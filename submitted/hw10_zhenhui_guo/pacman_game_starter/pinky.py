...
class Pinky(GameCharacter):
    ...
    def update(self):
        ...
        if on_vert and on_horz:
            if up_down_part and abs(up_down_part) > abs(left_right_part):
                sign = up_down_part/abs(up_down_part)
                self.y_add = sign * self.velocity
                self.x_add = 0
            else:
                self.y_add = 0
                sign = left_right_part/abs(left_right_part)
                self.x_add = sign * self.velocity
        # END CODE CHANGES
